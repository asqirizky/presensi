...
<div class="modal fade" id="kt_modal_new_target{{ $item->id }}" tabindex="-1" aria-hidden="true">
    ...
    <div class="modal-dialog modal-dialog-centered mw-800px">
        ...
        <div class="pt-0 modal-body scroll-y px-15 px-lg-15 pb-15">
            <!--begin:Form-->
            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('kehadiran-libur.update', $item->id) }}">
                @csrf
                @method('PUT')
                <!--begin::Heading-->
                ...
                <div class="mb-8 text-start fv-row">
                    ...
                    <div class="mb-8 fv-row">
                        <select class="form-select" name="ruang" value="" data-control="select2" data-hide-search="true" data-placeholder="Pilih Keterangan" required >
                            <option value="Perpustakaan Putra" {{ $item->ruang == 'Perpustakaan Putra' ? 'selected' : '' }}>Perpustakaan Putra</option>
                            <option value="Perpustakaan Putri" {{ $item->ruang == 'Perpustakaan Putri' ? 'selected' : '' }}>Perpustakaan Putri</option>
                            <option value="Semua Ruang" {{ $item->ruang == 'Semua Ruang' ? 'selected' : '' }}>Semua Ruang</option>
                        </select>
                    </div>
                    <!--end::Col-->
                </div>
                ...
                <div class="mb-8 fv-row text-start">
                    ...
                    <!--end::Label-->
                    <!--begin::Col-->
                    <input type="text" class="form-control form-control-lg" name="acara" value="{{ $item->acara }}" placeholder="" required>
                    <!--end::Col-->
                </div>
                <!--end::Input group-->
                <!--begin::Input group-->
