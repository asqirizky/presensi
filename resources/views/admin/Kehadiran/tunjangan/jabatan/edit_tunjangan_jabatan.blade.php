...
<div class="modal fade" id="kt_modal_new_target{{ $item->id }}" tabindex="-1" aria-hidden="true">
    ...
    <div class="modal-dialog modal-dialog-centered mw-800px">
        ...
        <div class="pt-0 modal-body scroll-y px-15 px-lg-15 pb-15">
            <!--begin:Form-->
            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('kehadiran-tunjangan-jabatan.update', $item->id) }}">
                @csrf
                @method('PUT')
                <!--begin::Heading-->
                ...
                <div class="mb-8 fv-row text-start">
                    ...
                    <div class="mb-8 fv-row">
                        <select class="mb-2 form-select" name="jabatan" data-control="select2" data-hide-search="true">
                        <option value="" disabled @selected(old('jabatan', $item->nama_jabatan ?? '') == '')>Pilih Jabatan</option>
                        @foreach ($jabatan as $j)
                            <option value="{{ $j->nama_jabatan }}"
                                    @selected(old('jabatan', $item->nama_jabatan ?? '') == $j->nama_jabatan)>
                                {{ $j->nama_jabatan }}
                            </option>
                        @endforeach
                    </select>
