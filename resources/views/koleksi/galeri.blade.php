    ...
    <div class="product-section-one mt-150 lg-mt-80 mb-150 lg-mb-60">
        <div class="container">
            <div class="shop-page-header d-lg-flex align-items-center justify-content-between">
                <p class="m0 md-pb-20">
                    Menampilkan
                    <span class="fw-500 text-dark">{{ $galeri->firstItem() }}–{{ $galeri->lastItem() }}</span>
                    dari
                    <span class="fw-500 text-dark">{{ $galeri->total() }}</span> hasil
                </p>
                    ...
                    <div class="col-lg-4 col-sm-6" data-aos="fade-up" data-aos-delay="100">
                        <div class="product-block-one mb-60 md-mb-40">
                            <div class="img-holder style-two">
                                <a href="{{ asset('/galeri='.$item->id) }}" class="d-flex align-items-center justify-content-center h-100">
                                    <img src="assets/images/lazy.svg" data-src="{{ asset('/storage/koleksi/galeri/' . $item->gambar) }}" alt="" class="lazy-img product-img tran4s w-100" style="height: 480px; object-fit: cover;">
                                </a>
                                <a href="{{ asset('/galeri='.$item->id) }}" class="cart-button">Selengkapnya</a>
                            </div> <!-- /.img-holder -->
                            <div class="product-meta">
                                <div class="d-lg-flex align-items-center justify-content-between">
                                    <a href="{{ asset('/galeri='.$item->id) }}" class="product-title fw-bold fs-4">{{ $item->judul }}</a>
                                </div>
                ...
                <div class="pagination-one mt-30 lg-mt-10">
                    <ul class="style-none d-flex align-items-center justify-content-center">
                        ...
                        @for ($i = 1; $i <= $galeri->lastPage(); $i++)
                            ...
                            @if ($i == 1 || $i == $galeri->lastPage() || ($i >= $galeri->currentPage() - 1 && $i <= $galeri->currentPage() + 1))
                                <li>
                                    <a href="{{ $galeri->url($i) }}" class="{{ $galeri->currentPage() == $i ? 'active' : '' }}">
                                        {{ $i }}
                                    </a>
                                </li>
                            @elseif ($i == 2 && $galeri->currentPage() > 4)
                                <li>...</li>
                            @elseif ($i == $galeri->lastPage() - 1 && $galeri->currentPage() < $galeri->lastPage() - 3)
                                <li>...</li>
                            @endif
                        @endfor
                        ...
                        @if ($galeri->currentPage() < $galeri->lastPage())
                            <li>
                                <a href="{{ $galeri->url($galeri->lastPage()) }}">
